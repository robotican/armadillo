...
# installation file for armadillo over ROS Kinetic and ubuntu 16.04 #
...
INSTALL_HW_COMPS=false #deremine if this script will install armadillo hardware drivers
...
printf "${WHITE_TXT}\n***Installing Armadillo ROS-Kinetic Package***\n${NO_COLOR}"
...
then  
    if [ $1 = "hw" ]
    then
        printf "${WHITE_TXT}\nGot hardware argument. Installing package for Armadillo hardware... ${NO_COLOR}\n"
...
if [ "$INSTALL_HW_COMPS" = true ] ; then
    ...
    cd $PKG_PATH/armadillo/third_party_files/
...
sudo apt-get -y install libglfw3-dev
cd $PKG_PATH/armadillo_utils/libfreenect2
mkdir build && cd build
cmake .. -DCMAKE_INSTALL_PREFIX=$HOME/freenect2
make
sudo make install
cd $PKG_PATH/armadillo_utils/iai_kinect2/iai_kinect2
rosdep install -r --from-paths .
printf "${GREEN_TXT}Done.\n\n${NO_COLOR}"

# install armadillo ric interface #
...
if [ "$INSTALL_HW_COMPS" = true ] ; then
    ...
    sudo cp $PKG_PATH/armadillo/rules/usb_to_dxl.rules /etc/udev/rules.d
    sudo cp $PKG_PATH/armadillo/rules/roboteq.rules /etc/udev/rules.d
    sudo cp $PKG_PATH/armadillo/rules/49-teensy.rules /etc/udev/rules.d
    sudo cp $PKG_PATH/armadillo/rules/bms_battery.rules /etc/udev/rules.d
    sudo cp $PKG_PATH/armadillo/rules/hokuyo.rules /etc/udev/rules.d/
    sudo cp $PKG_PATH/armadillo_utils/libfreenect2/platform/linux/udev/90-kinect2.rules /etc/udev/rules.d/
    sudo cp $PKG_PATH/armadillo/rules/99-realsense-libusb.rules /etc/udev/rules.d
    sudo udevadm control --reload-rules && udevadm trigger
    printf "${GREEN_TXT}Done.\n\n${NO_COLOR}"
fi

# compiling armadillo #
printf "${WHITE_TXT}Compiling armadillo package...\n${NO_COLOR}"
cd ~/catkin_ws
catkin_make
printf "${GREEN_TXT}Done.\n\n${NO_COLOR}"
